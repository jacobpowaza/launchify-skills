...
This skill implements the Launchify canonical specification.

**Spec location:** Check for `.launchify/` in the project directory first. If not found, use `~/.launchify/` (global install).

Before performing any action, read the following files in order (using whichever path exists):
